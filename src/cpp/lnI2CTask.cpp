...
class i2cTask : public lnI2cTask, public xTask
{
  public:
    ...
    virtual bool getCCLimited()
    {
        return _currentLimited;
    }

    virtual void setMaxCurrent(int mA)
    {
        _newMaxCurrentMa = mA;
    }
    virtual void setDCEnable(bool enable)
    {
        _newDCEnabled = enable;
    }
    virtual void setOutputEnable(bool enable)
    {
        _newOutputEnabled = enable;
    }
    void notify(lnI2cTask::SignalChange sign)
    {
        _accumulated |= sign;
    }
...
extern i2cTask *tsk;
float lnI2cTaskShim::getVoltage()
{
    return tsk->getVoltage();
};
int lnI2cTaskShim::getCurrent()
{
    return tsk->getCurrent();
};
void lnI2cTaskShim::setMaxCurrent(int mA)
{
    tsk->setMaxCurrent(mA);
};
bool lnI2cTaskShim::getCCLimited()
{
    return tsk->getCCLimited();
};
void lnI2cTaskShim::setDCEnable(bool enable)
{
    tsk->setDCEnable(enable);
};
void lnI2cTaskShim::setOutputEnable(bool enable)
{
    tsk->setOutputEnable(enable);
};
/**
 *
 */
